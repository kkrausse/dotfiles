...
** ob-duckdb
#+begin_src elisp
;; Load ob-duckdb from external file
(with-eval-after-load 'ob
  (load-file (expand-file-name "local-packages/ob-duckdb-local.el" doom-user-dir))
  (add-to-list 'org-babel-load-languages '(duckdb . t)))

;; Configure syntax highlighting for duckdb blocks as SQL
...
(after! command-log-mode
  ...
  (add-hook 'command-log-mode-hook
            (lambda ()
              (when (string= (buffer-name) "*command-log*")
                (command-log-mode -1))))
  )
...
(after! vterm
  ...
  ;; NOTE this was experiment. see if we could have more interractive term.
  ;; might wanna keep some of the escape / C-c stuff
  ;;
  ;; (setq vterm-keymap-exceptions '("C-x" "C-u" "C-g" "C-h" "C-l" "M-x" "M-o" "C-y" "M-y"))
  ;; (map! :map vterm-mode-map
  ;;       :i "<escape>" #'vterm--self-insert
  ;;       :i "<S-escape>" #'evil-normal-state)


  ;; temp for debug. do uncomment
...
;; idea:
;; turn on ultra key logging / debug info
;; - pastable into llm
;; - llm really good at debugging burried patterns / issues so figure whats up
;; bc it'd be too tedious to do low level debugging like that?

;; just map everywhere
(map!
 (:prefix ("SPC j" . "kevin stuff")
          :nv  "i" 'kev-indent
          :nv "t" 'projectile-term ;'my-switch-to-nearest-vterm
