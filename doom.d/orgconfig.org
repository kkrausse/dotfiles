...
(after! org-roam
  (cl-defmethod org-roam-node-kev-parent-path ((node org-roam-node))
    ...
    (let ((node-id (org-roam-node-id node)))
      (or (gethash node-id kev/org-roam-node-subnode-name-cache)
          (let ((result
                 (let ((parents (kev/subnode-parents node)))
                   (if parents
                       (concat
                        (s-join " -> "
                                (reverse
  ...
  (setq org-roam-node-display-template "${kev-parent-path}${title}")
  (setq kev/org-roam-node-subnode-name-cache (make-hash-table :test 'equal))
  )


(kev/comment
 (s-join "taco" (list "a" "b"))
 ;; NOTE:
...
(map! (:prefix ("C-c n" . "Org Roam")
               ...
               "f" #'org-roam-node-find ;; (cons "find node" #'kev/roam-subnode-find-non-subnodes)
