...
(defun projectile-term ()
  ...
  (let ((term-buf (seq-some (lambda (b)
                              (with-current-buffer b
                                (and (derived-mode-p 'term-mode) b)))
...
;; note, that with the advice, I disable it for clojure-cli
(setq cider-inject-dependencies-at-jack-in t)

(define-advice cider-inject-jack-in-dependencies (:around (orig-fun global-opts params project-type)
                                                          inject-deps-advice)
  (pcase project-type
    ('clojure-cli params)
    (_ (funcall orig-fun global-opts params project-type))))
