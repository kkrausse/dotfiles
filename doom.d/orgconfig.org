  ...
  (cl-defmethod org-roam-node-kev-subnode-format ((node org-roam-node))
    ...
    (let ((parent-node (thread-last node
      ...
      (if parent-node
        ...
        org-roam-node-display-template "${kev-subnode-format}"
  )

(defun kev/org-roam-find-all-nodes ()
  (interactive)
  (require 'org-roam)
  (let ((org-roam-node-display-template "${kev-subnode-format}"))
    (org-roam-node-find)))
...
(map! (:prefix ("C-c n" . "Org Roam")
               ...
               "f" #'kev/org-roam-find-all-nodes ;; (cons "find node" #'kev/roam-subnode-find-non-subnodes)
