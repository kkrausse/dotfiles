...
(defun kev/roam-subnode-create ()
  (interactive)
  (let ((parent (org-roam-node-read nil nil nil 'require-match))
...
(defun subnode-paths (tree)
  (or
   (thread-last (cdr tree)
                (mapcar (lambda (pt)
                          (concat (s-join " " (subnode-paths pt))
                                  " -> "
                                  (org-roam-node-title (car tree))))))
   (list (org-roam-node-title (car tree)))))
...
(defun kev/subnode-formatted-parents (node)
  (let* ((parent-paths
         ...
         (max-len (cl-reduce 'max
                             (mapcar 'length parent-paths)
                             :initial-value 0)))
    (thread-last parent-paths
                 ;; (mapcar (lambda (str)
                 ;;           (concat (make-string (- max-len (length str)) ?\s) str)))
                 (s-join "|"))))
...
(after! org-roam
  ...
  (setq org-roam-mode-sections (list #'kev/roam-dailies-section
